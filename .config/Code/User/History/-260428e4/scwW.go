...
func GetUserReactions(conn modules.Connection, uId, reaction string, db *sql.DB) {
	...
	query := `SELECT SUBSTRING_INDEX(item_id,'_',-1) FROM reactions WHERE user_id=? AND SUBSTRING_INDEX(item_id,'_',1='posts')`
...
query=`SELECT content, title,categories, created_at FROM posts WHERE = ? ORDER BY updated_at DESC`
	}
}
