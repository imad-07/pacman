...
func GetUserReactions(conn *modules.Connection, uId int, reaction string, db *sql.DB) {
	...
	for rows.Next() {
		...
		if err = db.QueryRow(query, postID).Scan(&post.Publisher.Id, &post.Publisher.Id, &post.Content.Text, &post.Content.Title, &post.Categories, &post.CreationTime); err != nil {
			conn.NewError(http.StatusInternalServerError, 500, "internal server error", "")
			return
		}
