...
func GetUSer(conn *modules.Connection, forumDB *sql.DB) {
	...
	case "like", "dislike":
		GetUserReactions(conn, conn.InternalUserId, conn.Path[3], forumDB)
	default:
...
func GetUserReactions(conn *modules.Connection, uId int, reaction string, db *sql.DB) {
	...
	for rows.Next() {
		...
		if err = db.QueryRow(query, postID).Scan(&post.Publisher.Id, &post.Publisher.Id, &post.Content.Text, &post.Content.Title, &post.CreationTime); err != nil {
			conn.NewError(http.StatusInternalServerError, 500, "internal server error", "")
			return
		}
		post.Publisher, err = posts.GetPublicUser(post.Publisher.Id, db)
		if err != nil {
			log.Warn(err)
		}
		post.Content.Categories, err = posts.GetPostCategories(post.Id, db)
		if err != nil {
			log.Warn(err)
		}
...
func GetUSerPosts(conn *modules.Connection, userId int, db *sql.DB) {
	...
	for rows.Next() {
		var post modules.Post
		if err := rows.Scan(&post.Publisher.Id, &post.Publisher.Id, &post.Content.Text, &post.Content.Title, &post.CreationTime); err != nil {
			conn.NewError(http.StatusInternalServerError, 500, "internal server error", "")
		}
