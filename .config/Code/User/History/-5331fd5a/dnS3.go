...
func fetchPostsByCategories(categories []string, posts *[]modules.Post, page int, forumDB *sql.DB) error {
	...
	query := `SELECT user_id, item_id, title, content, categories, created_at FROM posts   WHERE (CAST(categories AS INTEGER) & ?) != 0 ORDER BY created_at DESC   LIMIT ? OFFSET ?`
...
func fetchAllPosts(posts *[]modules.Post, page int, forumDB *sql.DB) error {
	...
	query := `SELECT user_id, item_id, title, content, categories, created_at   FROM posts   ORDER BY created_at DESC   LIMIT ? OFFSET ?`
