...
export default class GameBoard {
    ...
    objectExist = (character, object) => {
        const b = Math.floor(character.positionX / 20) + Math.floor(character.positionY / 20) * 20;
        const j = Math.floor(character.positionX / 20) + Math.floor(character.positionY / 20) * 20;
    
        if (character.moveUp) {
            const y = Math.floor((character.positionY - character.speed) / 20);
            if (this.grid[b] != 1 && this.grid[j] != 1) {
                this.grid[y][Math.floor(character.positionX / 20)].classList.remove("dot", "pill");
            }
            return this.grid[y][Math.floor(character.positionX / 20)].classList.contains(object);
            ...
            if (this.grid[b] != 1 && this.grid[j] != 1) {
                this.grid[Math.floor(character.positionY / 20)][x].classList.remove("dot", "pill");
            }
            return this.grid[Math.floor(character.positionY / 20)][x].classList.contains(object);
    
        } else if (character.moveRight) {
            const x = Math.ceil((character.positionX + character.speed) / 20);
            if (this.grid[b] != 1 && this.grid[j] != 1) {
                this.grid[Math.ceil(character.positionY / 20)][x].classList.remove("dot", "pill");
            }
            return this.grid[Math.ceil(character.positionY / 20)][x].classList.contains(object);
        }
    }
