...
func EmailIsUnique(email string) (bool, error) {
	exists := false
	query := `SELECT EXISTS (SELECT email FROM users WHERE email = ?)`

	if err := database.Db.QueryRow(query, email).Scan(&exists); err != nil {
		return true, err
	}
...
func UsernameIsUnique(username string) (bool, error) {
	...
	if err := database.Db.QueryRow(query, username).Scan(&exists); err != nil {
		return true, err
	}
